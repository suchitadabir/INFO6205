...
public class SlowSelectTest {
    ...
    public void testSelectWithIntegers() throws Exception {
        ...
        int k = 1;
        while (k <= a.length) {
            SlowSelect<Integer> ss = new SlowSelect<>(a, k);
            Integer result = ss.select(ss.xa, ss.k);
            assertArrayEquals(a, ss.xa);
    ...
    public void testSelectWithCharacters() throws Exception {
        ...
        int k = 1;
        while (k <= a.length) {
            SlowSelect<Character> ss = new SlowSelect<>(a, k);
            Character result = ss.select(ss.xa, ss.k);
            assertArrayEquals(a, ss.xa);
            assertEquals(b[k - 1], result);
            k++;
        }
    ...
    public void testSelectWithStrings() throws Exception {
        ...
        int k = 1;
        while (k <= a.length) {
            SlowSelect<String> ss = new SlowSelect<>(a, k);
            String result = ss.select(ss.xa, ss.k);
            assertArrayEquals(a, ss.xa);
            assertEquals(b[k - 1], result);
            k++;
        }
